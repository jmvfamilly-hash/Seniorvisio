...
/**
 * Modèle de reconnaissance vocale française utilisé par [VoskTranscriber],
 * partagé pour toute la durée de vie du process : charger un modèle Vosk
 * prend plusieurs secondes, il ne doit pas être refait à chaque appel/prise
 * de sous-titres.
 *
 ...
 * fonctionne hors-ligne et gratuitement (aucune clé API, aucun coût par
 * minute transcrite — contrairement à AssemblyAI/Deepgram utilisés
 * auparavant). [prepare] est appelé dès le démarrage du service d'écoute
 * permanent et de l'écran d'accueil pour maximiser les chances que le
 * modèle soit prêt avant le premier usage réel.
 */
    ...
    fun prepare(context: Context) {
        ...
        executor.execute {
            try {
                ...
                // Pas de réseau au premier lancement, modèle corrompu, etc. :
                // getModel() reste null, les appelants (VoskTranscriber)
                // signalent alors une erreur explicite plutôt que de planter.
