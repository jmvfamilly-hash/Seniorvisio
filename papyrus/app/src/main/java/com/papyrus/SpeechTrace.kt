...
object SpeechTrace {
    ...
    private fun header(): String = buildString {
        ...
        appendLine()
        appendLine("  Une session sur deux laisse le MOTEUR décider — aucune consigne de silence,")
        appendLine("  aucun plafond raisonnable, figeage sur son seul onResults, comme l'interface")
        appendLine("  d'Android le prévoit. L'autre session applique notre découpage par")
        appendLine("  événements. Les deux régimes sont donc dans la même trace, sur la même voix.")
        appendLine()
        appendLine("  « APP origine de la clôture »  À LIRE EN PREMIER sur les sessions API.")
        ...
        appendLine()
        appendLine("  « APP L'API N'A PAS CLOS »  la réponse inverse : une minute entière sans")
        appendLine("                          onResults. Si cette ligne apparaît sur toutes les")
        appendLine("                          sessions API, le séquencement par le moteur est")
        appendLine("                          impossible sur cet appareil, et la question est")
        appendLine("                          close.")
        appendLine()
        appendLine("  « APP bilan session »   porte le régime en tête, API ou ÉVÉNEMENTS. Comparer")
        appendLine("                          à voix égale : lignes figées, durée, délai du 1er")
        appendLine("                          texte. Un régime qui produit moins de lignes n'est")
        appendLine("                          pas forcément moins bon — encore faut-il regarder")
        appendLine("                          si ce sont des phrases ou des fragments.")
        appendLine()
