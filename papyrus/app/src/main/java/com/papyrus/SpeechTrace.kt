...
object SpeechTrace {
    ...
    private fun header(): String = buildString {
        ...
        appendLine()
        appendLine("  « 1er texte après »     délai entre « le moteur écoute » et le premier mot")
        appendLine("                          rendu. Mesuré de 2 à 8 s après chaque relance, et")
        appendLine("                          le moteur signale pourtant de la parole pendant ce")
        appendLine("                          temps : c'est de la surdité, pas du silence. C'est")
        appendLine("                          pour l'espacer que la session peut désormais durer")
        appendLine("                          deux minutes au lieu de vingt secondes.")
        appendLine()
        ...
        appendLine()
        appendLine("  ═══ L'EXPÉRIENCE EN COURS : qui séquence les phrases ? ═══")
        appendLine()
        appendLine("  Un seul régime par session d'essai, choisi au bouton en bas à droite.")
        appendLine("  La ligne « APP → startListening » le nomme au démarrage de chaque session,")
        appendLine("  et « ÉCRAN bascule » marque l'instant où la personne a changé de régime.")
        appendLine()
        appendLine("  API PURE : le moteur fait tout, on regarde. Aucune consigne de silence,")
        appendLine("  AUCUN plafond, aucun stopListening de notre part, aucun figeage déclenché")
        appendLine("  par une fin ou un début de parole. La phrase est écrite sur onResults.")
        appendLine()
        appendLine("  ÉVÉNEMENTS : notre découpage. Consignes de silence, plafond de deux")
        appendLine("  minutes, figeage sur la séquence « fin de parole puis début de parole ».")
        appendLine()
        appendLine("  « APP origine de la clôture »  À LIRE EN PREMIER sur les sessions API PURE.")
        appendLine("                          « SPONTANÉE » = le moteur a clos sa session de")
        appendLine("                          lui-même, et l'API sait séquencer. Le délai indiqué")
        appendLine("                          est ce qu'il fait attendre après la dernière fin de")
        appendLine("                          parole : c'est le prix de cette solution.")
        appendLine()
        appendLine("  L'ABSENCE DE LIGNES est ici un résultat, pas une panne. En API PURE, une")
        appendLine("                          session qui court sans jamais produire onResults ni")
        appendLine("                          relance signifie que le moteur ne clôt pas de")
        appendLine("                          lui-même. Chercher le dernier « APP → startListening »")
        appendLine("                          et regarder combien de temps la trace continue")
        appendLine("                          au-delà sans rien dire : c'est la mesure.")
        appendLine()
        appendLine("  « APP bilan session »   porte le régime en tête, API PURE ou ÉVÉNEMENTS,")
        appendLine("                          avec durée, lignes figées et délai du 1er texte.")
        appendLine("                          Un régime qui produit moins de lignes n'est pas")
        appendLine("                          forcément moins bon : encore faut-il regarder si ce")
        appendLine("                          sont des phrases ou des fragments.")
        appendLine()
        appendLine("  « APP frontière »       la séquence « fin de parole PUIS début de parole »,")
        appendLine("                          seule chose qui ferme une phrase désormais. Plus")
        appendLine("                          aucune minuterie, plus aucune comparaison de textes :")
        appendLine("                          les partiels ne font que remplacer le tampon, et ce")
        appendLine("                          sont les deux événements du moteur qui décident.")
        appendLine("                          Vérifier que chaque ligne écrite en découle, et")
        appendLine("                          mesurer l'écart entre la fin de parole et le début")
        appendLine("                          suivant : c'est le délai avant que le texte")
        appendLine("                          n'apparaisse (0,7 à 1,3 s sur l'appareil d'essai).")
        appendLine()
        appendLine("  « APP partiel reçu (non affiché) »  l'écran ne montre plus les révisions,")
        appendLine("                          seulement les lignes figées.")
        appendLine()
