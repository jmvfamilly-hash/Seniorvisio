...
object SpeechTrace {
    ...
    private fun header(): String = buildString {
        ...
        appendLine()
        appendLine("  « API onSegmentResults » le moteur déclare LUI-MÊME une frontière d'énoncé.")
        appendLine("                          À l'essai une session sur deux : comparer les lignes")
        appendLine("                          produites par les sessions SEGMENTÉ à celles des")
        appendLine("                          sessions classiques, sur la même voix. Si ce rappel")
        appendLine("                          n'apparaît jamais, le mode n'est pas supporté par ce")
        appendLine("                          moteur — la documentation prévient qu'il peut être")
        appendLine("                          ignoré sans erreur.")
        appendLine()
